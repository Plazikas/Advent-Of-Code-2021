...
dashboard = []
winnerDashboard = (len(balls_numbers), [], 0)
try:
    for linea in f:
        if linea != '\n':
            ...
            if infoDashboard[0] < winnerDashboard[0]:
                winnerDashboard = infoDashboard
                print('El resultado es: ' + str(ResultSum(winnerDashboard) * int(winnerDashboard[2])))
            dashboard.clear()
finally:
    f.close()
